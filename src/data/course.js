...
export const days = [
  ...
  },
  {
    ...
    skillBlock: {
      id: "taste-skill",
      title: "Taste Skill Workshop",
      timebox: "90 min",
      topics: [
        "What Taste Skill is and the anti-slop problem it solves",
        "Installing design-taste-frontend into OpenCode, Cursor, Claude Code, and v0",
        "How a SKILL.md file rewrites AI agent behavior at inference time",
        "Real audit: applying Taste Skill rules to this bootcamp repo",
        "Student task: improve your own portfolio website using any LLM",
      ],
      activity:
        "Install the design-taste-frontend skill, run a real before/after audit on the bootcamp repo, then refactor your own portfolio website with it.",
      deliverable:
        "A short before/after of your portfolio (GitHub Pages URL, screenshots, or a 30-sec screen recording) and 1 commit that added the SKILL.md to your repo",
      steps: [
    ...
    modules: [
      {
        ...
        topics: [
          ...
          "Self-audit and peer-review for citation discipline",
        ],
        activity: "Summarise the UNESCO Recommendation on the Ethics of Artificial Intelligence into a 1-page brief, audit every claim against the source, and peer-review a partner's.",
        deliverable: "executive-brief.md with peer-review stamp",
        challenge: null,
        example:
          "Read the UNESCO Recommendation on the Ethics of AI (adopted by all 193 Member States in November 2021). Compress it into a 1-page exec brief covering TL;DR (3 lines), Key Findings (5), Risks (3), and Recommendations (3) — then audit every claim against the page numbers in the source PDF and rewrite any sentence you cannot source.",
        steps: [
          "Open the source: download the official PDF from https://unesdoc.unesco.org/ark:/48223/pf0000381137 (English). The full Recommendation is 36 pages; the short summary at https://unesdoc.unesco.org/ark:/48223/pf0000385082 is a faster on-ramp.",
          "Instructor demo (10 min): live-summarise the Recommendation using Gemini in front of the room. Show how to anchor the brief to specific paragraphs and quote only what the source actually says.",
          "Ask the model for: TL;DR (3 lines), Key Findings (5), Risks (3), Recommendations (3). Make Findings traceable to the 4 core values and the 10 core principles in the source — not to a generic 'AI ethics' prompt.",
          "Rewrite the brief in your own words. The model is good at paraphrasing; your job is to verify.",
          "Self-audit: re-read the brief and underline any claim you cannot point to a page or paragraph in the source for. Cut anything uncited. Common hallucinations: invented policy-area names, fake article numbers, paraphrases that contradict the source.",
          "Peer review: trade briefs with a partner. Partner marks every claim as ✓ cited (page/paragraph), ❌ uncited, or ⚠️ inferred. Author revises.",
          "Stretch (fast finishers): rewrite the same brief as a 3-tweet thread and a 30-second elevator pitch aimed at a government audience that has never heard of UNESCO.",
          "Verify every claim against the source PDF before submitting. If a claim is in your brief but not in the Recommendation, it does not ship.",
        ],
      },
      {
        id: "m8",
        ...
        topics: [
          ...
          "Sensitivity testing on weighted scoring",
          "Cross-team critique and Q&A defence",
        ],
        activity: "Generate a SWOT, risk matrix, and 3-option recommendation, then defend it in a 2-min pitch.",
        deliverable: "decision-pack.md with SWOT + risk matrix + recommendation + 2-min defence script",
        challenge: "Decision Defense",
        example:
          "Decision: 'Should our team adopt Ollama for offline AI?' Build the SWOT for Ollama, LM Studio, and a cloud-only path. Score the 3 options on cost, privacy, latency, dev velocity, and lock-in. Run a sensitivity test by shifting one weight ±30% and see if the recommendation flips. Then defend the call in 2 min.",
        steps: [
          "Instructor walkthrough (10 min): SWOT + 5×5 risk matrix + weighted scoring sheet on the projector using the Ollama adoption scenario.",
          "Pick a real decision (e.g. 'Should we adopt Ollama for offline AI?').",
          "Individual brainstorm: generate your own SWOT using RTCF.",
          "Pair merge: combine two SWOTs into one, eliminate duplicates, rank the top 5 risks.",
          "Build a 5×5 risk matrix (likelihood × impact) for the top 5 risks.",
          "Compare 3 options with criteria scoring (1-5) and weighted totals.",
          "Sensitivity test: change one weight by ±30%. If the recommendation flips, note why it is fragile.",
          "Stretch (fast finishers): convert the decision pack into a 1-page Google Slides deck — title, the matrix, the recommendation, one risk.",
          "Table read-throughs: 3 teams of 4 defend their recommendation in 2 min each, with 1 min of Q&A. Facilitator times strictly.",
          "Cross-team critique: each non-presenting team writes one 'strongest evidence' and one 'biggest blind spot' sticky for the presenting team.",
      ...
      },
      {
        ...
        topics: [
          ...
          "Backlog grooming and OpenCode dry-runs",
        ],
        activity: "Turn a project idea into a TASKS.md backlog an AI agent could execute, then dry-run it.",
        deliverable: "TASKS.md with 20+ actionable tasks and OpenCode dry-run log",
        challenge: null,
        example:
          "For the Day-4 Meeting Summariser app: write 20 tasks across setup, data, model, UI, testing, and docs. Sample tasks — 'Set up Ollama endpoint', 'Create sample transcript fixture', 'Build /summarise POST handler with input validation', 'Add 10 unit tests for edge cases'. Hand TASKS.md to OpenCode and ask it to sequence the first 3.",
        steps: [
          "Instructor demo (10 min): show a strong vs. weak TASKS.md side-by-side. Demonstrate handing the good one to OpenCode and watching it plan the next 3 tasks.",
          "Pick the offline app you'll build on Day 4 (Meeting Summariser, FAQ Assistant, etc.).",
          "Brainstorm 20+ tasks across: setup, data, model, UI, testing, docs.",
          "For each task write: title, description, acceptance criteria, and dependency.",
          "Acceptance-criteria pass: cut any task without clear AC.",
          "Stretch (fast finishers): add a Definition of Done section at the top of TASKS.md covering testing, docs, and demo.",
          "Hand TASKS.md to OpenCode and ask it to plan the next 3 tasks. Mark which of its suggestions you accept, modify, or reject.",
          "Group critique: facilitator projects 3 TASKS.md files. Cohort votes on which are agent-ready vs vague.",
        ],
        tool: "tasksBuilder",
      },
    ],
...
export const schedule = [
  ...
  { day: 3, block: "Taste Skill Workshop", minutes: 90 },
  { day: 3, block: "Recap & Module 7 — Executive Summary Sprint", minutes: 45 },
  { day: 3, block: "Morning break", minutes: 15 },
  { day: 3, block: "Module 8 — Decision Matrix Challenge", minutes: 105 },
  { day: 3, block: "Lunch", minutes: 60 },
  { day: 3, block: "Module 9 — Agent Planning Workshop", minutes: 45 },
  { day: 3, block: "Afternoon break", minutes: 15 },
  { day: 3, block: "Hallucination Hunt debrief", minutes: 30 },
  { day: 3, block: "Decision Defense challenge", minutes: 60 },
  { day: 3, block: "Day 3 close", minutes: 15 },
...
export const glossary = [
  ...
  { term: "Decision Defense", def: "Day 3 challenge: teams defend a recommendation generated with a SWOT, risk matrix, and weighted scoring in a timed pitch with Q&A." },
  { term: "Sensitivity test", def: "Shifting one weight in a scoring matrix by ±30% to see if the recommendation still holds. If it flips, the decision is fragile." },
